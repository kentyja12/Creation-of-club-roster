...
def gakuban_append_flow(app): 
    ...
    if type(student_data.df_meibo) == str: # 名簿が読み込まれているかチェック
        ...
        Static3 = tkinter.Label(textvariable=app.msg)
        Static3.place(x=X,y=Y)
    elif not any(student_data.df_meibo["学籍番号"].isin([gakuban_tmp])): # 読み込んだ名簿上に学籍番号があるかチェック 
        app.msg.set("あなたの学籍番号は名簿上にありません。\n部外者の場合は、インスタント名簿登録を行ってください。")
        Static3 = tkinter.Label(textvariable=app.msg)
        Static3.place(x=X,y=Y)
    elif gakuban_tmp in student_data.gakuban_list: # 登録予定のリストに既にあるかチェック
        app.msg.set("{}は既に登録されています。".format(gakuban_tmp))
        Static3 = tkinter.Label(textvariable=app.msg)
        Static3.place(x=X,y=Y) 
    else: # リストに学籍番号の格納
        student_data.gakuban_list.append(gakuban_tmp)
        app.msg.set("{}の登録に成功しました。".format(gakuban_tmp))
        Static3 = tkinter.Label(textvariable=app.msg)
        Static3.place(x=X,y=Y) 
